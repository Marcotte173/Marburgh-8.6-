...
{    
    ...
    {
        new Recipe(magPrefix[1],0,3,0,0, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.DAMAGE,3,true),
        new Recipe(magPrefix[2],0,1,1,1, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.HEALTH,10,true),
        new Recipe(magPrefix[3],0,2,3,2, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.GOLD,15,true),
        new Recipe(magPrefix[4],0,4,1,0, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.XP,5,true),
        new Recipe(magSuffix[1],0,2,0,2, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.DAMAGE,3,false),
        new Recipe(magSuffix[2],0,1,0,1, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.HEALTH,10,false),
        new Recipe(magSuffix[3],0,2,3,2, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.GOLD,15,false),
        new Recipe(magSuffix[4],0,4,1,0, Drop.drop[0], Drop.drop[1], Drop.drop[2],Drop.drop[3],80, EnhancementType.XP,5,false)
    };
    ...
    {
        Console.Clear();
        //This is poorly made. To revisit
        ...
        if (p.craft == false)
            ...
            {
                Colour.TIME + "S" + Colour.RESET, "", craftButton
            }, null);
        else UI.General(
            new List<int> { 1, 1, 0 }, 
            new List<string>
        ...
        else if (choice == "s")
        {
            Console.Clear();
            UI.General(
                new List<int> { 0, 2, 1, 0, 0},new List<string>
                {
        ...
        if (choice == "x")
        {
            ...
            {
                if (p.Drops[i] == Drop.drop[1]) p.Drops[i].amount++;
            }
            if (!p.Drops.Contains(Drop.drop[1])) p.Drops.Add(Drop.drop[1]);
        }
    ...
    private static void ItemChange(Creature p, Equipment item)
    {
        ...
        do
        {
            ...
            {                
                ...
                else Utilities.EmbedColourText(Colour.magicColour[(int)CraftMachineEnhancements[i].type + 1], $"\n[{i + 1}] ", $"{CraftMachineEnhancements[i].name}", "");
