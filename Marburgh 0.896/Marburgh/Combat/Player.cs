...
public class Player
{
    ...
    public static void PlayerDeathCheck(Creature p, Monster mon, string killedBy)
    {
        if (p.health == 0)
        {
            if (mon == null)
            {
                Utilities.ColourText(Colour.DAMAGE, $"You died!\n");
                Family.timeOfDeath[Family.DeadSiblings.Count, 0] = Time.day;
                Family.timeOfDeath[Family.DeadSiblings.Count, 1] = Time.week;
                Family.timeOfDeath[Family.DeadSiblings.Count, 2] = Time.month;
                Family.timeOfDeath[Family.DeadSiblings.Count, 3] = Time.year;
            }
            else
            {
                Family.timeOfDeath[Family.DeadSiblings.Count, 0] = Time.day;
                ...
                Family.timeOfDeath[Family.DeadSiblings.Count, 2] = Time.month;
                Family.timeOfDeath[Family.DeadSiblings.Count, 3] = Time.year;
                string a = (mon.name.FirstOrDefault() == 'A' || mon.name.FirstOrDefault() == 'E' || mon.name.FirstOrDefault() == 'I' || mon.name.FirstOrDefault() == 'O' || mon.name.FirstOrDefault() == 'U') ? "an " + Colour.MONSTER + $"{mon.name}" + Colour.RESET : "a " + Colour.MONSTER + $"{mon.name}" + Colour.RESET;
                Family.killingMonster.Add(a);
